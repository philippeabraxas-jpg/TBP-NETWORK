...
[apercu.md](apercu.fr.md). Revenir ici pour l'ordre d'installation, la
...
|---|---|---|---|
| Routeur | Debian 12 dédiée (ou VM, 2+ interfaces) | NAC 802.1X (FreeRADIUS EAP-TLS), nftables, VLANs §5.1 | [router-debian.md](router-debian.fr.md) |
| Cellule | VM par cellule (a, b, …) | broker, registre tessera, OPA, tracker d'époques, HSM (genèse §12) | [cellule.md](cellule.fr.md) |
...
closed ne se demande qu'après [monitor-to-closed.md](monitor-to-closed.fr.md).
...
- **Trous déclarés** : `opa run --capabilities` a disparu en OPA ≥ 1.0 :
  ...
  documentée dans [cellule.md](cellule.fr.md) et exécutée par le selftest.
  Le scénario netns/FreeRADIUS complet se joue en lab
  (`lab/containerlab/`), pas en sandbox — les causes réelles sont citées
  dans [router-debian.md](router-debian.fr.md). Les démons `brokerd` et
