...
## Note technique v1.4.7 · 16 septembre 2026 · Philippe Collet
...
**Le traducteur n'est pas l'élément de sécurité** (doctrine ci-dessus) : ce n'est jamais lui qui juge une action sûre ou dangereuse — c'est l'action *produite* qui est jugée par les règles, sans exception (default-deny, §1). Une erreur de traduction ne contourne donc jamais le jugement : elle change quelle action lui est soumise, pas si cette action, une fois soumise, est correctement jugée. Le résidu n'est donc pas « la sécurité contournée par une mauvaise traduction » — les règles s'appliquent, sans faille, à ce que le traducteur produit réellement, aussi imparfait ce produit soit-il. Le résidu réel est plus étroit et déjà nommé : une action mal classée (mauvais domaine, tag manquant — §11.8) peut manquer la règle écrite pour sa vraie classe, quand cette règle est conditionnée à ce champ plutôt qu'au contenu direct de l'action. Le défaut-deny (§1) borne ce risque pour l'action non couverte (inconnu = strict) ; il ne le borne pas pour l'action couverte mais mal étiquetée, classée à tort dans une catégorie moins stricte que la sienne. C'est cette classification, pas le jugement lui-même, qui reste la partie la moins mûre du système (métriques ci-dessus, §15) — et la lourdeur vérifiable du reste de la chaîne (dry-run diff, object-capabilities, pattern-analysis §4.4, Merkle, multisig) ne doit jamais se lire comme si elle réduisait ce résidu-là : elle ne porte pas sur la classification, seulement sur ce qui en découle.
