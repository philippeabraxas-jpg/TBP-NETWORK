...
## Note technique v1.4.6 · 16 septembre 2026 · Philippe Collet
...
**OPA n'est jamais contourné — l'input l'est** : le PEP applique le default-deny (§1) à toute action, sans exception ; ce n'est pas la chaîne d'application qui est en cause (dry-run diff, object-capabilities, pattern-analysis §4.4, Merkle, multisig — tous corrects et auditables sur ce qu'on leur soumet). Le résidu est en amont : OPA décide sur l'action *traduite* (doctrine ci-dessus), jamais sur l'action réellement demandée si le traducteur ou la classification (§11.8) se sont trompés. Aussi lourde et vérifiable soit-elle, la chaîne prouve alors une chose vraie mais insuffisante — *le système a fait ce que l'entrée disait* — pas que l'entrée décrivait correctement ce qui a été demandé. Nommé pour ce qu'il est : plus l'appareil cryptographique en aval est visible, plus il peut donner l'illusion de couvrir aussi la qualité de traduction en amont — il ne la couvre pas. C'est elle, pas la chaîne d'application, qui reste la partie la moins mûre du système (métriques ci-dessus, §15).
...
12. La lourdeur vérifiable de la chaîne d'application (§3-§7, §4.4) ne réduit pas le risque de traduction/classification erronée (§4.5) — elle le rend seulement auditable a posteriori. Une décision d'application correcte et prouvable, appliquée à une action mal traduite, reste une décision correcte sur une mauvaise description de l'action : la sécurité perçue de l'appareil ne doit jamais se lire comme une preuve de qualité de traduction, ce sont deux choses distinctes.
