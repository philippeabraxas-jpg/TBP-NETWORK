...
## Note technique v1.4.9 · 18 septembre 2026 · Philippe Collet
...
**Séquence** : (1) HSM + cérémonie de genèse ; (2) fencing du cluster — émission et rotation des époques, quorum des contrôleurs (k-of-n) pour la classe W, promotion miroir/canari (§7) — requis avant tout déploiement multi-cellules, y compris le pilote P1 à 2 cellules ci-dessous ; une cellule unique peut différer cette étape, un pilote non ; (3) OPA + validateur + registre ; (4) PEP HTTP/gRPC = premier périmètre réellement gouverné ; (5) NAC en parallèle ; (6) traducteur + escalade F/I/W en dernier.
